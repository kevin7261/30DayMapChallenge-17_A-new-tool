<script>
   ...
   *    ✓ 白色縣市邊界線
   *    ✓ Google 衛星圖底圖（真實地形影像）
   *    ✓ 真實地形高度（使用 Terrarium DEM 開放數據，1:1 真實比例）
   *    ✓ 核電廠位置標記（紅色圓點，半徑 4 像素）
   *    ✓ 核電廠 30 公里紅色圓圈（平面圓圈，線寬 2px，透明度 80%）
      ...
      const addNuclearPlants = () => {
        ...
        const plants = [
          // 核電廠陣列
          {
            ...
            fullName: '核一廠（已除役）',
            status: '已除役',
            lon: 121.58572785873663,
            lat: 25.282252575377854,
          }, // 核一廠
          {
            name: '核二廠',
            fullName: '核二廠（已除役）',
            status: '已除役',
            lon: 121.65959500379523,
            lat: 25.20654889306922,
          }, // 核二廠
          {
            name: '核三廠',
            fullName: '核三廠',
            status: '運轉中',
            lon: 120.75176441457805,
            lat: 21.9585756506926,
          }, // 核三廠
          {
            name: '核四廠',
            fullName: '核四廠（封存中）',
            status: '封存中',
            lon: 121.92468538719159,
            lat: 25.038917511630075,
          }, // 核四廠
        ]; // plants 陣列結束
        ...
        // 創建 30 公里半徑的平面紅色圓圈 GeoJSON 數據
        const circlesData = {
          // GeoJSON 格式：FeatureCollection
          type: 'FeatureCollection', // type: 數據類型為 FeatureCollection
          features: plants.map((plant) => ({
            // features: 為每個核電廠創建一個圓圈 Feature
            type: 'Feature', // type: 要素類型
            properties: {
              // properties: 要素屬性
              name: plant.name, // name: 核電廠名稱
            ...
            geometry: {
              // geometry: 幾何形狀
              type: 'Polygon', // type: 多邊形類型
              coordinates: [createCirclePolygon(plant.lon, plant.lat, 30000)], // coordinates: 創建半徑 30km 的圓形座標
            }, // geometry 結束
          })), // map 結束
        }; // circlesData 結束

        // 添加 30 公里紅色圓圈數據源
        map.addSource('nuclear-circles', {
          // addSource: 添加數據源
          type: 'geojson', // type: 數據源類型為 GeoJSON
          data: circlesData, // data: 圓圈 GeoJSON 數據
        }); // addSource 結束

        // 添加 30 公里紅色圓圈圖層
        map.addLayer({
          // addLayer: 添加圖層
          id: 'nuclear-circles-layer', // id: 圖層唯一標識符
          type: 'line', // type: 圖層類型為 line（線條）
          source: 'nuclear-circles', // source: 使用圓圈數據源
          paint: {
            // paint: 繪製屬性
            'line-color': '#FF0000', // 'line-color': 線條顏色設為紅色
            'line-width': 2, // 'line-width': 線條寬度設為 2 像素
            'line-opacity': 0.8, // 'line-opacity': 線條透明度設為 0.8（80%，清晰可見）
          }, // paint 屬性結束
        }); // addLayer 結束：30 公里紅色圓圈圖層
        ...
        try {
          ...
          map.addLayer({
            ...
            paint: {
              ...
              'line-color': '#FFFFFF', // 'line-color': 線條顏色設為白色
              'line-width': 2, // 'line-width': 線條寬度設為 2 像素
            }, // paint 屬性結束
          }); // addLayer 結束：邊界線圖層
      ...
      const createMap = () => {
        ...
        try {
          ...
          map = new maplibregl.Map({
            ...
            style: {
              // style: 地圖樣式配置對象（使用 Google 衛星圖作為底圖）
              version: 8, // version: MapLibre 樣式規範版本號
              sources: {
                // sources: 數據源配置
                'google-satellite': {
                  // 'google-satellite': Google 衛星圖數據源
                  type: 'raster', // type: 數據源類型為 raster（柵格圖磚）
                  tiles: [
                    'https://mt0.google.com/vt/lyrs=s&x={x}&y={y}&z={z}',
                    'https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}',
                    'https://mt2.google.com/vt/lyrs=s&x={x}&y={y}&z={z}',
                    'https://mt3.google.com/vt/lyrs=s&x={x}&y={y}&z={z}',
                  ], // tiles: Google 衛星圖 URL 模板（使用多個服務器以提高加載速度）
                  tileSize: 256, // tileSize: 圖磚大小為 256x256 像素
                }, // 'google-satellite' 數據源結束
                'terrarium-dem': {
                  // 'terrarium-dem': Terrarium 數字高程模型（DEM）數據源
                  type: 'raster-dem', // type: 數據源類型為 raster-dem（數字高程模型）
                  tiles: [
                    'https://s3.amazonaws.com/elevation-tiles-prod/terrarium/{z}/{x}/{y}.png',
                  ], // tiles: Terrarium DEM 圖磚 URL
                  encoding: 'terrarium', // encoding: 編碼格式為 terrarium
                  tileSize: 256, // tileSize: 圖磚大小為 256x256 像素
                  maxzoom: 15, // maxzoom: 最大縮放級別為 15
                }, // 'terrarium-dem' 數據源結束
              }, // sources 結束
              layers: [
                // layers: 圖層配置數組
                {
                  // Google 衛星圖圖層
                  id: 'google-satellite-layer', // id: 圖層唯一標識符
                  type: 'raster', // type: 圖層類型為 raster（柵格圖磚）
                  source: 'google-satellite', // source: 使用的數據源 ID，對應上面定義的 'google-satellite'
                  minzoom: 0, // minzoom: 最小縮放級別
                  maxzoom: 22, // maxzoom: 最大縮放級別
                }, // Google 衛星圖圖層結束
              ], // layers 結束
            }, // style 結束
          ...
          map.on('load', () => {
            ...
            // 設置地形高度（使用 Terrarium DEM 數據源）
            map.setTerrain({
              // setTerrain: 設置地形
              source: 'terrarium-dem', // source: 使用 terrarium-dem 數據源
              exaggeration: 1, // exaggeration: 高度誇張係數，1 表示使用真實高度，不放大
            }); // setTerrain 結束
            console.log('[MapTab] 地形高度已啟用（Terrarium DEM，真實高度）'); // 輸出日誌：記錄地形啟用
