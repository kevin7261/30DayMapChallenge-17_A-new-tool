<script>
  ...
  export default {
    ...
    setup(_, { emit }) {
      ...
      /**
       * ⚛️ 添加核電廠標記和影響範圍圓
       * 在地圖上標記台灣4座核電廠的位置，並繪製半徑30公里的影響範圍圓
       */
      const addNuclearPlants = () => {
        // 定義核電廠 GeoJSON 數據（包含4座核電廠的位置和資訊）
        ...
        // 添加核電廠影響範圍圓圖層（半徑 30 公里）
        map.addLayer({
          // addLayer: 添加圖層
          id: 'nuclear-range-circles', // id: 圖層唯一標識符
          type: 'circle', // type: 圖層類型為 circle（圓形）
          source: 'nuclear-plants', // source: 使用的數據源 ID
          paint: {
            // paint: 繪製屬性
            'circle-radius': {
              // 'circle-radius': 圓的半徑（像素）
              stops: [
                // stops: 根據縮放級別調整半徑大小
                [0, 0], // [縮放級別, 半徑像素]: 縮放級別 0 時半徑為 0 像素
                [5, 20], // 縮放級別 5 時半徑為 20 像素
                [10, 100], // 縮放級別 10 時半徑為 100 像素
                [15, 400], // 縮放級別 15 時半徑為 400 像素
              ], // stops 結束：讓圓的大小隨縮放級別變化，在不同縮放級別下都能看到
            }, // 'circle-radius' 屬性結束
            'circle-color': '#FF0000', // 'circle-color': 圓的顏色設為紅色（警示色）
            'circle-opacity': 0.2, // 'circle-opacity': 圓的不透明度設為 0.2（20%，半透明）
            'circle-stroke-width': 2, // 'circle-stroke-width': 圓的邊框寬度設為 2 像素
            'circle-stroke-color': '#FF0000', // 'circle-stroke-color': 圓的邊框顏色設為紅色
            'circle-stroke-opacity': 0.8, // 'circle-stroke-opacity': 圓的邊框不透明度設為 0.8（80%）
          }, // paint 屬性結束
        }); // addLayer 結束：影響範圍圓圖層
        ...
        // 添加滑鼠懸停效果：當滑鼠移動到核電廠標記時顯示詳細資訊
        map.on('mousemove', 'nuclear-plants-points', (e) => {
          // map.on: 監聽事件; 'mousemove': 滑鼠移動事件; 'nuclear-plants-points': 監聽標記點圖層
          map.getCanvas().style.cursor = 'pointer'; // 將滑鼠游標設為 pointer（手型）

          if (e.features && e.features.length > 0) {
            // 檢查事件是否包含要素
            const feature = e.features[0]; // 獲取第一個要素
            const name = feature.properties.fullName || '未知核電廠'; // 從屬性中獲取完整名稱

            // 創建並顯示 popup（顯示核電廠詳細資訊）
            new maplibregl.Popup({ closeButton: false, closeOnClick: false }) // 創建 Popup
              .setLngLat(e.lngLat) // 設置 popup 位置為滑鼠坐標
              .setHTML(`<strong>${name}</strong><br>影響範圍：30 公里`) // 設置 popup 內容：名稱和影響範圍
              .addTo(map); // 將 popup 添加到地圖
          } // if 結束
        }); // mousemove 事件監聽結束

        // 添加滑鼠離開效果：當滑鼠離開核電廠標記時移除 popup
        map.on('mouseleave', 'nuclear-plants-points', () => {
          // map.on: 監聽事件; 'mouseleave': 滑鼠離開事件
          map.getCanvas().style.cursor = ''; // 恢復滑鼠游標為默認樣式

          // 移除所有 popup
          const popups = document.getElementsByClassName('maplibregl-popup'); // 獲取所有 popup 元素
          while (popups.length) {
            // 當還有 popup 存在時
            popups[0].remove(); // 移除第一個 popup
          } // while 結束
        }); // mouseleave 事件監聽結束

        console.log('[MapTab] 核電廠標記和影響範圍圓添加完成'); // 輸出日誌：記錄添加完成
      ...
      const add3DCountyLayers = () => {
        ...
        try {
          ...
          // 添加滑鼠懸停效果：當滑鼠移動到縣市上時顯示縣市名稱（使用最簡單的 popup 方式）
          map.on('mousemove', 'counties-3d', (e) => {
            // map.on: 監聽事件; 'mousemove': 滑鼠移動事件; 'counties-3d': 監聽此圖層; (e): 事件對象
            map.getCanvas().style.cursor = 'pointer'; // 將滑鼠游標設為 pointer（手型），表示可交互; getCanvas(): 獲取地圖 canvas 元素

            if (e.features && e.features.length > 0) {
              // 檢查事件是否包含要素（features）且數量大於 0
              const countyName = e.features[0].properties.COUNTYNAME || '未知縣市'; // 從第一個要素屬性中獲取 COUNTYNAME，如果不存在則使用 '未知縣市'

              // 創建並顯示簡單的 popup（每次重新創建，最簡單的方式）
              new maplibregl.Popup({ closeButton: false, closeOnClick: false }) // 創建 Popup：無關閉按鈕，點擊不關閉
                .setLngLat(e.lngLat) // 設置 popup 位置為滑鼠坐標
                .setHTML(countyName) // 設置 popup 內容為縣市名稱（純文字，最簡單）
                .addTo(map); // 將 popup 添加到地圖
            } // if 結束：要素檢查
          }); // mousemove 事件監聽結束

          // 添加滑鼠離開效果：當滑鼠離開縣市時移除所有 popup
          map.on('mouseleave', 'counties-3d', () => {
            // map.on: 監聽事件; 'mouseleave': 滑鼠離開事件; 'counties-3d': 監聽此圖層
            map.getCanvas().style.cursor = ''; // 恢復滑鼠游標為默認樣式（箭頭）

            // 移除所有 popup（使用 querySelectorAll 選擇所有 popup 元素並移除）
            const popups = document.getElementsByClassName('maplibregl-popup'); // 獲取所有 popup 元素
            while (popups.length) {
              // 當還有 popup 存在時
              popups[0].remove(); // 移除第一個 popup（移除後數組長度自動減 1）
            } // while 結束：移除所有 popup
          }); // mouseleave 事件監聽結束
