    ...
    setup(_, { emit }) {
      ...
      const addNuclearPlants = () => {
        ...
        sphereLayersData.forEach((layerData, index) => {
          ...
          // 計算當前層的透明度（因為層多且重疊，降低每層透明度以保持整體效果）
          const heightRatio = index / (layers - 1); // heightRatio: 當前層高度比例（0 到 1）
          const opacity = 0.12 - heightRatio * 0.06; // opacity: 透明度，底層 0.12，頂層 0.06（非常透明，避免重疊過濃）
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'line-opacity': 0.6, // 'line-opacity': 線條透明度設為 0.6（60%）
          }, // paint 屬性結束
        }); // addLayer 結束：邊界線圖層
      ...
      const createMap = () => {
        ...
        try {
          ...
          map = new maplibregl.Map({
            ...
            style: {
              ...
              sources: {
                // sources: 數據源配置
                'osm-tiles': {
                  // 'osm-tiles': OpenStreetMap 圖磚數據源
                  type: 'raster', // type: 數據源類型為 raster（柵格圖磚）
                  tiles: ['https://tile.openstreetmap.org/{z}/{x}/{y}.png'], // tiles: 圖磚 URL 模板，{z}/{x}/{y} 為圖磚坐標參數
                  tileSize: 256, // tileSize: 圖磚大小為 256x256 像素
                  // attribution: 版權聲明文字
                  attribution:
                    '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors', // OpenStreetMap 版權聲明，包含超連結
                }, // 'osm-tiles' 數據源結束
              }, // sources 結束
              layers: [
                // layers: 圖層配置數組
                {
                  ...
                  paint: {
                    // paint: 繪製屬性
                    'background-color': '#808080', // 'background-color': 背景顏色設為灰色
                  }, // paint 結束
                }, // 背景圖層結束
                {
                  // 第二個圖層：OpenStreetMap 圖磚圖層
                  id: 'osm-tiles-layer', // id: 圖層唯一標識符
                  type: 'raster', // type: 圖層類型為 raster（柵格圖磚）
                  source: 'osm-tiles', // source: 使用的數據源 ID，對應上面定義的 'osm-tiles'
                  minzoom: 0, // minzoom: 最小縮放級別，0 表示世界級別
                  maxzoom: 22, // maxzoom: 最大縮放級別，22 表示街道級別
                }, // OpenStreetMap 圖磚圖層結束
              ], // layers 結束
            }, // style 結束
          ...
          // 添加導航控制器（縮放按鈕和指南針）
          map.addControl(new maplibregl.NavigationControl(), 'top-right'); // addControl: 添加控制器; NavigationControl: 導航控制器（包含縮放和旋轉按鈕）; 'top-right': 控制器位置（右上角）
